...
public class CalculateExecutionWorker implements JobHandler {
  ...
  public CalculateExecutionWorker(WorkerConfig workerConfig, MonitorWorker monitorWorker) {
  ...
  public void handle(JobClient jobClient, ActivatedJob job) throws Exception {
    ...
    long endCampaign = monitorWorker.endCampaign();
    ...
    long currentExecution = endCampaign - startDate;

    logger.info(
        "------------- Worker: calculateExecutionWorker PID[" + processInstanceKey + "] Type[" + typeWorker + "] in "
            + monitorWorker.getSynthesis() + " duration==campaign ? " + (currentExecution
            == monitorWorker.getDurationCampaign()));
